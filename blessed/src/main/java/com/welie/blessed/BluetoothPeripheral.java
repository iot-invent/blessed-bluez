...
import static com.welie.blessed.BluetoothCommandStatus.AUTHENTICATION_FAILURE;
import static com.welie.blessed.BluetoothCommandStatus.BLUEZ_NOT_READY;
import static com.welie.blessed.BluetoothCommandStatus.BLUEZ_OPERATION_FAILED;
import static com.welie.blessed.BluetoothCommandStatus.BLUEZ_OPERATION_IN_PROGRESS;
import static com.welie.blessed.BluetoothCommandStatus.COMMAND_SUCCESS;
import static com.welie.blessed.BluetoothCommandStatus.CONNECTION_ALREADY_EXISTS;
import static com.welie.blessed.BluetoothCommandStatus.CONNECTION_FAILED_ESTABLISHMENT;
import static com.welie.blessed.BluetoothCommandStatus.CONNECTION_REJECTED_SECURITY_REASONS;
import static com.welie.blessed.BluetoothCommandStatus.DBUS_EXECUTION_EXCEPTION;
import static com.welie.blessed.BluetoothCommandStatus.INSUFFICIENT_AUTHENTICATION;
import static com.welie.blessed.BluetoothCommandStatus.INSUFFICIENT_AUTHORIZATION;
import static com.welie.blessed.BluetoothCommandStatus.INVALID_ATTRIBUTE_VALUE_LENGTH;
import static com.welie.blessed.BluetoothCommandStatus.INVALID_COMMAND_PARAMETERS;
import static com.welie.blessed.BluetoothCommandStatus.INVALID_OFFSET;
import static com.welie.blessed.BluetoothCommandStatus.NOT_CONNECTED;
import static com.welie.blessed.BluetoothCommandStatus.READ_NOT_PERMITTED;
import static com.welie.blessed.BluetoothCommandStatus.REMOTE_USER_TERMINATED_CONNECTION;
import static com.welie.blessed.BluetoothCommandStatus.REQUEST_NOT_SUPPORTED;
import static com.welie.blessed.BluetoothCommandStatus.WRITE_NOT_PERMITTED;
...
public final class BluetoothPeripheral {
	...
	final GattCallback gattCallback = new GattCallback() {
		...
		@Override
		public void onDescriptorRead(@NotNull final BluetoothGattDescriptor descriptor, @NotNull final byte[] value, @NotNull final BluetoothCommandStatus status) {
	...
	public void connect() {
		...
		} catch (final BluezAlreadyConnectedException e) {
		...
		} catch (final BluezInProgressException e) {
	...
	public boolean readCharacteristic(@NotNull final BluetoothGattCharacteristic characteristic) {
		...
		return enqueue(() -> {
			if (state == CONNECTED) {
				try {
					logger.debug(String.format("reading characteristic <%s>", nativeCharacteristic.getUuid()));
				...
				} catch (final BluezInvalidOffsetException e) {
		...
		return enqueue(() -> {
			if (state == CONNECTED) {
				try {
					...
					logger.debug(String.format("writing %s <%s> to characteristic <%s>", writeType, bytes2String(bytesToWrite), nativeCharacteristic.getUuid()));
				...
				} catch (final BluezInvalidValueLengthException e) {
			...
			if (state == CONNECTED) {
				try {
					if (enable) {
						...
						final boolean isNotifying = nativeCharacteristic.isNotifying();
						if (isNotifying) {
	...
	public void readRemoteRssi() {
		try {
			...
			final Short rssi = Objects.requireNonNull(device).getRssi();
			if (rssi != null) {
	...
	private final AbstractPropertiesChangedHandler propertiesChangedHandler = new AbstractPropertiesChangedHandler() {
		...
		public void handle(@NotNull final Properties.PropertiesChanged propertiesChanged) {
			...
			case BLUEZ_CHARACTERISTIC_INTERFACE:
				final BluetoothGattCharacteristic bluetoothGattCharacteristic = getCharacteristicFromPath(propertiesChanged.getPath());
	...
	private BluezGattDescriptor getBluezGattDescriptor(@NotNull final UUID serviceUUID, @NotNull final UUID characteristicUUID, @NotNull final UUID descriptorUUID) {
		...
		final BluezGattCharacteristic characteristic = getBluezGattCharacteristic(serviceUUID, characteristicUUID);
		if (characteristic != null) {
	...
	private BluezGattCharacteristic getBluezGattCharacteristic(@NotNull final UUID serviceUUID, @NotNull final UUID characteristicUUID) {
		...
		for (final BluezGattCharacteristic gattCharacteristic : characteristicMap.values()) {
	...
	public boolean createBond(@NotNull final BluetoothPeripheralCallback peripheralCallback) {
		...
		try {
			...
			return true;
		} catch (final BluezInvalidArgumentsException e) {
	...
	private void startServiceDiscoveryTimer() {
		...
		final Runnable timeoutRunnable = () -> {
			final String deviceName = device != null ? device.getName() : deviceAddress;
	...
	private BluetoothGattCharacteristic mapBluezGattCharacteristicToBluetoothGattCharacteristic(final BluezGattCharacteristic bluezGattCharacteristic) {
		final int properties = mapFlagsToProperty(bluezGattCharacteristic.getFlags());
	...
	private BluetoothGattService mapBluezGattServiceToBluetoothGattService(@NotNull final BluezGattService service) {
		...
		characteristics.forEach(bluetoothGattCharacteristic -> {
			...
			final BluetoothGattCharacteristic characteristic = mapBluezGattCharacteristicToBluetoothGattCharacteristic(bluetoothGattCharacteristic);
